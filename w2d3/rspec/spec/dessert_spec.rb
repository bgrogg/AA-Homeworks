...
describe Dessert do
  let(:chef) { double("chef", name: "Curry") }
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Curry with the pot")
      expect(cake.serve).to eq("Chef Curry with the pot has made 50 cakes!")
    end
  end
